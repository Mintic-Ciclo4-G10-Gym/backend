...
public class EjercicioController {
    ...
    @Autowired
    EjercicioService servicio;

    @PostMapping("/ejercicio")
    public ResponseEntity<Map <String, String>> guardar(@Valid @RequestBody EjercicioModel entidad, Errors error){
        if(error.hasErrors()){
            throwError(error);
        }
    ...
    public EjercicioModel buscar(@PathVariable String id){
    ...
    public ResponseEntity<Map<String, String>> actualizar(@RequestBody EjercicioModel entidad, Errors error){
        if(error.hasErrors()){
            throwError(error);
        }
