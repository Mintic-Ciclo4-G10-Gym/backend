...
public class SedeController {
    ...
    @Autowired
    SedeService servicio;

    @PostMapping("/sede")
    public ResponseEntity<Map <String, String>> guardar(@Valid @RequestBody SedeModel entidad, Errors error){
        if(error.hasErrors()){
            throwError(error);
        }
    ...
    @GetMapping("/sede/{id}")
    public SedeModel buscar(@PathVariable String id){
    ...
    @PutMapping("/sede")
    public ResponseEntity<Map<String, String>> actualizar(@RequestBody SedeModel entidad, Errors error){
        if(error.hasErrors()){
            throwError(error);
        }
